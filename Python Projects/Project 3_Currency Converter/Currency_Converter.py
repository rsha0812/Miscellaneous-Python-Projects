...
class CurrencyConverter():
    #To get real time exchange rate and convert the currency.
    def __init__(self,url):
        self.data= requests.get(url).json() # to load page and convert into json file
    ...
    def convert(self, from_currency, to_currency, amount):
        ...
        #from_currency = USD
        #to_currency = enter by user

        # limiting the precision to 4 decimal places
        amount = round(amount * self.currencies[to_currency], 4)
        return amount #return to converted amount
...
class CurrencyConverterUI(tk.Tk):
    def __init__(self, converter):
        ...
        self.currency_converter = converter #creating UI Frame

        self.geometry("500x200")

        #Label
        self.intro_label = Label(self, text = 'Welcome to Real Time Currency Convertor',  fg = 'blue', relief = tk.RAISED,borderwidth = 3)
        self.intro_label.config(font = ('Courier',15,'bold','italic'))

        self.date_label = Label(self, text = f"1 Indian Rupee equals = {self.currency_converter.convert('INR','USD',1)} USD \n Date : {self.currency_converter.data['date']}", fg = 'green', relief = tk.GROOVE, borderwidth = 5)
